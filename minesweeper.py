...
class Board:
    ...
    def getAdjacentList(self, tile):
        adjacentPositions = [Position(tile.position.x - 1,tile.position.y - 1),
        Position(tile.position.x, tile.position.y - 1),
        Position(tile.position.x + 1, tile.position.y - 1),
        Position(tile.position.x - 1, tile.position.y),
        Position(tile.position.x + 1, tile.position.y),
        Position(tile.position.x - 1, tile.position.y + 1),
        Position(tile.position.x, tile.position.y + 1),
        Position(tile.position.x + 1, tile.position.y + 1)]
    ...
    def isBomb(self, position):
        if(position.x >= 0 and position.x < self.difficulty.size and position.y >= 0 and position.y < self.difficulty.size):
    ...
    def chooseTile(self, position):
        if(self.tiles[position.x][position.y].isClicked == False):
            ...
            if(self.tiles[position.x][position.y].closeBombsCounter == 0):
                self.flipTiles(self.tiles[position.x][position.y])

    def checkTileIsInRangeAndUntouched(self, tile, tilesToBeFlipped):
        if(tile.x >= 0 and tile.x < self.difficulty.size and 
           tile.y >= 0 and tile.y < self.difficulty.size and 
           self.tiles[tile.x][tile.y] not in tilesToBeFlipped and 
           self.tiles[tile.x][tile.y].isClicked == False):
            return 1
        else:
            return 0
        ...
        while(len(tilesToBeFlipped) > 0):
            ...
            for i in adjacentPositions:
                if(self.checkTileIsInRangeAndUntouched(i, tilesToBeFlipped)):
                    self.tiles[i.x][i.y].isClicked = True
                    self.numberOfTilesClicked += 1
                    if(self.tiles[i.x][i.y].closeBombsCounter == 0):
                        tilesToBeFlipped.append(self.tiles[i.x][i.y])
